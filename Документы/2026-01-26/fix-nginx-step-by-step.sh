#!/bin/bash
# Пошаговое исправление конфигурации Nginx на сервере
# Выполняйте команды по одной на сервере

# Шаг 1: Проверить текущую конфигурацию
cat /etc/nginx/sites-available/welcomebaby.neetrino.com

# Шаг 2: Если конфигурация неполная, создать правильную
cat > /etc/nginx/sites-available/welcomebaby.neetrino.com << 'ENDOFFILE'
# Редирект HTTP → HTTPS
server {
    listen 80;
