...
## Обновление без Docker (PM2, порт 3000)

Короткий чеклист для сервера `http://localhost:3000`.
...
Частые проблемы:
- Изображения категорий 404 после загрузки:
  - Используйте веб‑путь `/images/<file>.jpg` (не `blob:`/`data:`)
  - При сохранении категории автоматически добавляется `?v=<timestamp>`
- Prisma/BД: проверьте `.env` → `DATABASE_URL`, затем `npx prisma migrate deploy`
- Порт занят: `pm2 stop all && pm2 delete all` или `sudo lsof -ti:3000 | xargs -r sudo kill -9`